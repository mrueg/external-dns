...
func testNewNetcupProvider(t *testing.T) {
	p, err := NewNetcupProvider(10, "KEY", "PASSWORD", []string{"example.com"}, true)
	assert.NotNil(t, p.client)
	assert.NoError(t, err)

	_, err = NewNetcupProvider(0, "KEY", "PASSWORD", []string{"example.com"}, true)
	assert.Error(t, err)

	_, err = NewNetcupProvider(10, "", "PASSWORD", []string{"example.com"}, true)
	assert.Error(t, err)

	_, err = NewNetcupProvider(10, "KEY", "", []string{"example.com"}, true)
	assert.Error(t, err)

	_, err = NewNetcupProvider(10, "KEY", "PASSWORD", []string{}, true)
	assert.Error(t, err)

}

func testApplyChanges(t *testing.T) {
	p, _ := NewNetcupProvider(10, "KEY", "PASSWORD", []string{"example.com"}, true)
