...
func NewNetcupProvider(customerID int, apiKey string, apiPassword string, zones []string, dryRun bool) (*NetcupProvider, error) {
	if len(zones) == 0 {
		return nil, fmt.Errorf("netcup provider needs at least one configured DNS Zone")
	}
